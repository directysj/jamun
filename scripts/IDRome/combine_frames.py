...
def combine_frames(name: str, input_dir: str, original_traj_dir: str, output_dir: str) -> None:
    ...
    traj_AA = None
    frames = sorted(os.listdir(os.path.join(input_dir, name)), key=lambda name: int(name.split("_")[0]))
    for frame in frames:
    ...
    traj_AA = md.Trajectory(
        traj_AA.xyz, top_AA, original_traj.time, original_traj.unitcell_lengths, original_traj.unitcell_angles
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input-dir",
        help="Directory of relaxed all-atom trajectories (stored in each folder).",
        type=str,
        required=True,
    )
    parser.add_argument(
        "--original-traj-dir",
        help="Directory of original coarse-grained trajectories (stored in each folder).",
        type=str,
        required=True,
    )
    parser.add_argument(
        "--output-dir",
        "-o",
        help="Output directory to save combined relaxed all-atom trajectories (stored in each folder).",
        type=str,
        required=True,
    )
